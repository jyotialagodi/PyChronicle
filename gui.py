...
def timeline():
    try:
        subprocess.run(["python", "timeline_viewer.py"], check=True)
    except Exception as e:
        messagebox.showerror("Error", str(e))
...
tk.Button(root, text="Run Program",
          width=30, height=2,
          command=run_program).pack(pady=8)

tk.Button(root, text="Execution Timeline",
          width=30, height=2,
          command=timeline).pack(pady=8)

tk.Button(root, text="Variable History",
          width=30, height=2,
          command=history).pack(pady=8)

tk.Button(root, text="Snapshot Viewer",
          width=30, height=2,
